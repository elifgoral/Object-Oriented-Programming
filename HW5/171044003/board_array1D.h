#ifndef BOARD_ARRAY1D_H
#define BOARD_ARRAY1D_H
...
namespace puzzleOfElif{
	...
	class BoardArray1D : public AbstractBoard{
		public:
			...
			BoardArray1D(int b_row,int b_column);//2 parameter constructor
			~BoardArray1D(); //destructor
			BoardArray1D(const BoardArray1D& object); //copy constructor
			BoardArray1D& operator=(const BoardArray1D& rightSide); // assignment operator
			void initRandomly(); // it inits randomly the board.
			...
			void initRespectively(int *array); 
